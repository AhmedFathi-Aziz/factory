...
import { Shield, Award, Fish } from "lucide-react";
...
export function ProductsList() {
  ...
  return (
    <section className={`py-16 sm:py-20 bg-gray-50 min-h-screen ${isRTL ? "rtl-spacing" : ""}`} dir={isRTL ? "rtl" : "ltr"}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <h1 className={`text-3xl sm:text-4xl font-bold text-primary mb-4 ${isRTL ? "font-arabic" : ""}`}>{t("ourProducts")}</h1>
          <p className={`text-lg sm:text-xl text-gray-600 max-w-3xl mx-auto ${isRTL ? "font-arabic" : ""}`}>{t("productsDescription")}</p>
        </motion.div>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 mb-8 sm:mb-12">
          {products.map((product, index) => (
            ...
            >
              <Card className="h-full hover:shadow-xl transition-shadow duration-300 border-0 shadow-lg bg-white animate-fade-in">
                <CardContent className={`p-6 sm:p-8 ${isRTL ? "rtl-padding" : ""}`}>
                  ...
                  <h3 className={`text-xl sm:text-2xl font-bold text-gray-900 mb-4 ${isRTL ? "font-arabic" : ""}`}>{language === "ar" ? product.titleAr : product.title}</h3>
                  <p className={`text-gray-600 mb-6 leading-relaxed ${isRTL ? "font-arabic" : ""}`}>{language === "ar" ? product.descriptionAr : product.description}</p>
                  <div className="mb-6">
                    ...
                    <ul className={`space-y-3 ${isRTL ? "rtl-spacing" : ""}`}>
                      {(language === "ar" ? product.featuresAr : product.features)?.map((feature: string, idx: number) => (
                        <li key={idx} className={`text-sm text-gray-600 flex items-center ${isRTL ? "font-arabic" : ""}`}>
                          <div className={`w-2 h-2 bg-blue-600 rounded-full ${isRTL ? "ml-3" : "mr-3"} flex-shrink-0`} />
                          {feature}
                        </li>
                      ))}
                    </ul>
